    ...
    <>
      <div className="top">
        <div className="inputs">
          <input type="text" placeholder="Enter a Pokémon" ref={inputRef} />
          <button
            onClick={() => {
      ...
      <div className="flex">
        <div className="list">
          ...
          <ul>{weaknesses && weaknesses.map((w) => <li key={w}>{w}</li>)}</ul>
        </div>
        <div>{img && <img src={img} alt={`Sprite for ${pokemon}`} />}</div>
        <div className="list">
          {resistances && `Resistances`}
          <ul>{resistances && resistances.map((r) => <li key={r}>{r}</li>)}</ul>
        </div>
      </div>
    </>
  );
